...
function tree(param) {
    ...
    d3.json("treeData?name=" + param.name + "&tree=" + param.tree, function (error, flare) {
        console.log(flare)
        //关闭加载层
        layer.closeAll();
        if (flare != null) {
            ...
            function update(source) {
                ...
                var nodeEnter = node.enter().append("g")
                    ...
                    .on("mouseover", d => {
                        ...
                        // 更新浮层内容
                        chartTooltip.select(".name").text("用户：" + d.name);
                        chartTooltip.select(".dept").text("部门：" + d.dept);
                        // 移除浮层hidden样式，展示浮层
